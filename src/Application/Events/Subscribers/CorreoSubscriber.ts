...
export class CorreoSubscriber implements IEventSubscriber {
    ...
    public async manejar(evento: Evento): Promise<void> {
        ...
        if (evento.nombre === 'DEPOSITO_REALIZADO') {
            ...
            if (idDestino) {
                try {
                    const cDestino = await this.cuentaRepo.buscarPorId(idDestino);
                    if (cDestino) {
                        cuentaDestinoTexto = cDestino.obtenerNumeroCuenta().toString();
                    }
                } catch {
                    // Si falla la búsqueda por ID, mantenemos la cuenta pasada
                }
            }
